...
namespace UserBusinessLayer.BusinessModel.Editables
{
    ...
    public class EditableUser: BusinessBase<EditableUser>
    {
        ...
        private void DataPortal_Delete(int id)
        {
            using(var context = new UserDataLayer.DbContext.UserDbContext())
            {
                var user = context.User.Include("Watchlists").Include("Friends").Include("Achievements").Where(x => x.Id == id).FirstOrDefault();
                if (user == null)
                ...
                if (user.Achievements != null) {
                    ...
                    achievs.AddRange(user.Achievements);
                    foreach (var item in achievs)
                    {
                        user.Achievements.Remove(item);
                    }
                }
                if(user.Friends != null)
                {
